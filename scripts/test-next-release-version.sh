...
assert_output "first build" 'version=1.2.3-1' 1.2.3 HEAD
...
assert_output "increment highest build" 'version=1.2.3-4' 1.2.3 HEAD
git -C "$repo" tag desktop-v1.2.3-4
assert_output "reuse current commit tag" 'existing=true' 1.2.3 HEAD
assert_output "reset counter for new base" 'version=2.0.0-1' 2.0.0 HEAD

if (cd "$repo" && ./scripts/next-release-version.sh 1.2 HEAD >/dev/null 2>&1); then
